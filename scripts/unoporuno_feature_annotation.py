...
"""
      ...
      usage:
      python unoporuno_feature_annotation.py (search name|search number)
...
from dospordos.features import RegexFeature, FeatureError, GazetteerFeature
try:
...
CountryGazet = GazetteerFeature(UNOPORUNO_ROOT, 'country')
CityGazet = GazetteerFeature(UNOPORUNO_ROOT, 'city', False)
CityGazetCase = GazetteerFeature(UNOPORUNO_ROOT, 'city', True)
...
for p in busqueda.persona_set.all():
    ...
    for s in p.snippet_set.all():
        ...
        if OrganizationRegex.test(descr_test_str):
            ...
            s.RE_features |= 1
        if CountryGazet.test(title_test_str):
            logging.debug ('FOUND COUNTRY IN TITLE:\n ' +s.title)
            s.RE_features |= 2
        if CountryGazet.test(descr_test_str):
            logging.debug('FOUND COUNTRY IN DESCR:\n ' +s.description)
            s.RE_features |= 2
        if CityGazetCase.test(title_test_str):
            logging.debug ('FOUND CITY IN TITLE:\n ' +s.title)
            s.RE_features |= 4
        if CityGazetCase.test(descr_test_str): 
            logging.debug('FOUND CITY IN DESCR:\n ' +s.description)
            s.RE_features |= 4
        if CityGazet.test(title_test_str):
            logging.debug ('FOUND CITY IN TITLE:\n ' +s.title)
            s.RE_features |= 4
        if CityGazet.test(descr_test_str):
            logging.debug('FOUND CITY IN DESCR:\n ' +s.description)
            s.RE_features |= 4
